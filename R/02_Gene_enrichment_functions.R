...
run_overrepresentation_analysis <- function(gene_set, dds_res, 
  ...
  if(type == "reactome"){
      ...
      entrez_ids.df <- clusterProfiler::bitr(geneID = geneList.filtered, fromType = "ENSEMBL", toType = "ENTREZID", OrgDb="org.Dr.eg.db")
      msig_h.oa <- enrichPathway(gene = entrez_ids.df$ENTREZID, qvalueCutoff = adj_p_cutoff, readable = TRUE, organism = "zebrafish")
    } else {
  ...
  if(dim(msig_h.oa@result)[1] > 0){
      ...
      msig_h.oa.gs <- setReadable(msig_h.oa, org.Dr.eg.db, keyType = "ENSEMBL")
...
run_enrichment_analysis <- function(gene_set, geneList, 
  ...
  if(type == "reactome"){
    # Replace ensembl IDs by Entrez IDs for compatibility with gsePathway
    entrez_ids.df <- clusterProfiler::bitr(geneID = names(geneList), fromType = "ENSEMBL", toType = "ENTREZID", OrgDb="org.Dr.eg.db")
    geneList.entrez <- geneList[c(entrez_ids.df$ENSEMBL)]
    names(geneList.entrez) <- entrez_ids.df$ENTREZID
    msig_h.gsea <- gsePathway(geneList = geneList.entrez, organism = "zebrafish", pvalueCutoff = adj_p_cutoff, verbose = TRUE, eps = 0)
    if(dim(msig_h.gsea@result)[1] > 0){ 
      # Convert Entrez gene IDs to gene symbols
      msig_h.gsea.gs <- setReadable(msig_h.gsea, org.Dr.eg.db, keyType = "ENTREZID")
    ...
    if(dim(msig_h.gsea@result)[1] > 0){ 
      # Convert Entrez gene IDs to gene symbols
      msig_h.gsea.gs <- setReadable(msig_h.gsea, org.Dr.eg.db, keyType = "ENSEMBL")
      if(no_plots){return(msig_h.gsea.gs)}
    }  
  } 
  ...
  if(dim(msig_h.gsea@result)[1] > 0){ 
    ...
    if(dim(msig_h.gsea)[1] > 1){ # We need at least 2 nodes for a network or tree plot
      ...
      # Treeplots
      number_of_categories = min(80, as.vector(table(distance_matrix@result$p.adjust < adj_p_cutoff)[['TRUE']]))
      # p12 <- treeplot(distance_matrix, showCategory = 80, 
...
go_overrep <- function(dds_res, my_file, golevel = 3, 
  ...
  #rownames(dds_res) <- replace_gene_acc_by_gb_ids(rownames(dds_res), return_all = TRUE)
  dds_res$entrez_ids <- replace_gene_acc_by_gb_ids(rownames(dds_res), return_all = TRUE)
  
  # Discard genes missing Entrez Gene IDs
  keep <- str_starts(string = dds_res$entrez_ids, pattern = "ENS")
  dds_res <- dds_res[!keep,]
  
  geneList.filtered <- subset(dds_res, abs(dds_res$log2FoldChange) >= log2FC_cutoff & dds_res$padj <= qvalue)
  
  # Remove duplicated entrez ids
  geneList.filtered <- geneList.filtered[!base::duplicated(geneList.filtered$entrez_ids), ]
  ...
  ego <- enrichGO(gene = geneList.filtered$entrez_ids,
                  OrgDb= "org.Dr.eg.db",
...
go_classification <- function(dds_res, my_file, golevel = 3, onthology = "BP", log2FC_cutoff = 0, alpha = 1){
  ...
  geneList.filtered <- subset(dds_res, abs(dds_res$log2FoldChange) >= log2FC_cutoff & dds_res$padj <= alpha)
  
  # Remove duplicated entrez ids
  geneList.filtered <- geneList.filtered[!base::duplicated(geneList.filtered$entrez_ids), ]
  
  print(paste( "Total number of Entrez gene IDs = ",nrow(geneList.filtered)))
  
  ggo <- groupGO(gene     = geneList.filtered$entrez_ids,
                 OrgDb    = "org.Dr.eg.db",
...
go_gsea <- function(dds_res, my_file, golevel = 3, 
  ...
  # Sort genes by absolute Log2FC (decreasing order)
  dds_res <- dds_res[order(abs(dds_res$log2FoldChange), decreasing = TRUE ), ]
  
  # Remove duplicated entrez ids
  dds_res.filtered <- dds_res[!base::duplicated(dds_res$entrez_ids), ]
  ...
  geneList = list()
  dds_res.sorted <- dds_res.filtered[order(dds_res.filtered$log2FoldChange, decreasing = TRUE ), ]
  geneList <- dds_res.sorted$log2FoldChange
  names(geneList) <- dds_res.sorted$entrez_ids
  ...
  go.gsea.gs <- setReadable(go.gsea , org.Dr.eg.db, keyType = "ENTREZID")
  ...
  if(length(go.gsea.gs@result$NES) > 0){
    ...
    if(dim(go.gsea.gs)[1] > 1){ # We need at least 2 nodes for a network or tree plot
      ...
      p6 <- emapplot(distance_matrix, 
                     ...
                     color = "NES", 
                     cex_label_category = 0.4,
                     node_label = "category", label_format = 20) #+ scale_edge_width(range = c(0.1,2)) 
